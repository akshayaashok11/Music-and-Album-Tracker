...
namespace DatabaseSQLMusicApp
{
    internal class AlbumsDAO
    {
        ...
        public List<Track> getTracksForAlbum(int albumID)
        {
            ...
            //define the sql statement to fetch all albums
            MySqlCommand command = new MySqlCommand();
            command.CommandText = "SELECT ID,track_title,Number,Video_URL,Lyrics,albums_id FROM TRACKS WHERE albums_ID = @albumid";
        ...
        public List<string> getVideoUrlsForAlbum(int albumID)
        {
            ...
            // Define the SQL statement to fetch video URLs for tracks
            MySqlCommand command = new MySqlCommand();
            command.CommandText = "SELECT `video_url` FROM `tracks` WHERE albums_ID = @albumid";
            command.Parameters.AddWithValue("@albumid", albumID);
            command.Connection = connection;
            ...
            using (MySqlDataReader reader = command.ExecuteReader())
            {
                while (reader.Read())
                {
                    string videoUrl = reader.GetString(0);
        ...
        internal int addOneTrack(Track track)
        {
            ...
            //define the sql statement to fetch all albums
            MySqlCommand command = new MySqlCommand("INSERT INTO `tracks`(`ID`, `TRACK_TITLE`, `NUMBER`,`VIDEO_URL`,`LYRICS`,`ALBUMS_ID`) VALUES(@trackID, @trackName, @number,@videoURL, @lyrics,@albumsid)", connection);
